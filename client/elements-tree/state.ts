...
import { elements$, ROOT_ID } from '../@app/state'
import { ca } from '../@app/supply/action-helpers'
import { isArrayEqual } from '../@app/supply/utils'
...
export const tree$ = elements$.pipe(
	scan<
		{
			[id: string]: IElement
		},
...
export const hoveredID$ = hoverElement.$.pipe(
	startWith<string | null>(null),
	shareReplay(1)
)

export const focusedID$ = actionsTree.focus.$.map(({ id }) => id).pipe(
	startWith<string | null>(null),
	shareReplay(1)
)

export const setTreeEl = ca<HTMLDivElement | null>()
export const treeEl$ = setTreeEl.$.pipe(
	startWith<HTMLDivElement | null>(null),
	shareReplay(1)
)
...
export const draggingID$ = merge(dndStartDragging.$, dndEndDragging.$.map(() => null)).pipe(
	startWith<string | null>(null),
	shareReplay(1)
)
...
export const placeholderParentID$ = dndState$
